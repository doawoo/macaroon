defmodule MacaroonVerificationTest do
  ...
  describe "First Party Verification" do
    setup do
      m = Macaroon.create_macaroon(@location, @id, @key)
    ...
    test "should verify when all exact first-party caveats are met (Verification.satisfy_exact/1..2)", context do

      result = Verification.satisfy_exact("test = caveat")
    ...
    test "should NOT verify when all exact first-party caveats are NOT met (Verification.satisfy_exact/1..2)", context do
      result = VerifyParameters.build() |> Verification.verify(context.m, @key)

      assert {:error, _result} = result
    end

    test "should verify when all general first-party caveats are met (Verification.satisfy_general/1..2)", context do
      result = Verification.satisfy_exact("test = caveat")
  ...
  describe "Third-Party Verification" do
    setup do
      ...
      m = Macaroon.create_macaroon(location, pub_id, key)
      |> Macaroon.add_first_party_caveat(account_number)
      |> Macaroon.add_third_party_caveat(tp_location, tp_pub_id, tp_key)

      discharge = Macaroon.create_macaroon(tp_location, tp_pub_id, tp_key)
    ...
    test "should verify when all exact third-party caveats are met (Verification.satisfy_exact/1..2)", context do
      result = Verification.satisfy_exact("account = 1234")
    ...
    test "should NOT verify when all exact third-party caveats are NOT met (Verification.satisfy_exact/1..2)", context do
      result = Verification.satisfy_exact("account = 1234")
      |> Verification.verify(context.m, context.key, [context.discharge])
    ...
    test "should NOT verify when all exact third-party when discharges are missing (Verification.satisfy_exact/1..2)", context do
      result = Verification.satisfy_exact("account = 1234")
