defmodule Macaroon.Verification do
  ...
  def verify(
        %VerifyParameters{} = verification_params,
        %Types.Macaroon{} = macaroon,
        key,
        discharge_macaroons \\ []
      )
      when is_binary(key) and is_list(discharge_macaroons) do
    ...
    result =
      if result.calculated_signature != macaroon.signature do
        error =
          VerifyError.build(
  ...
  def satisfy_exact(predicate) when is_binary(predicate) do
    VerifyParameters.build(predicates: [predicate])
  end

  @spec satisfy_general(Macaroon.Types.Verification.VerifyParameters.t(), fun) ::
          Macaroon.Types.Verification.VerifyParameters.t()
  def satisfy_general(%VerifyParameters{} = verification_context, callback)
      when is_function(callback) do
    %VerifyParameters{
      verification_context
      | callbacks: [callback | verification_context.callbacks]
    }
  end

  @spec satisfy_exact(Macaroon.Types.Verification.VerifyParameters.t(), binary) ::
          Macaroon.Types.Verification.VerifyParameters.t()
  def satisfy_exact(%VerifyParameters{} = verification_context, predicate)
      when is_binary(predicate) do
    %VerifyParameters{
      verification_context
      | predicates: [predicate | verification_context.predicates]
    }
  end

  defp verify_discharge(
         %Types.Macaroon{} = root_macaroon,
         %Types.Macaroon{} = macaroon,
         %VerifyContext{} = ctx
       ) do
  ...
  defp do_verify_third_party_caveat(
         %Types.Caveat{} = caveat,
         %Types.Macaroon{} = macaroon,
         %VerifyContext{} = ctx
       ) do
    discharge_macaroon =
      Enum.find(ctx.discharges, fn %Types.Macaroon{} = m ->
        m.public_identifier == caveat.caveat_id
      end)
